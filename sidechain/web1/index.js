...
.controller('MyCtrl', function($scope, $timeout) {
    ...
    $scope.getMerklePath = null;
    $scope.getMerklePath = function(txnobj){
        var res = aelf.chain.getMerklePath(txnobj.hash);
        txnobj.merkleproof = res;
        if(res.hasOwnProperty("error")){
            $timeout(function(){
                $scope.getMerklePath(txnobj);
            }, 4000);
        }
    }
